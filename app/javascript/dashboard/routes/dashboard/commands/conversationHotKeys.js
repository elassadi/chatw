...
const HANDOVER_CONVERSATION_ACTION = {
  id: 'handover_conversation',
  title: 'handover conversation to a rebot ',
  section: 'COMMAND_BAR.SECTIONS.CONVERSATION',
  icon: ICON_SEND_TRANSCRIPT,
  handler: () => bus.$emit(CMD_HANDOVER_CONVERSATION),
};
...
export default {
  ...
  computed: {
    ...
    handoverActions() {
      const chatStoryEntries = [{
        id: `chatstory-1`,
        title: "Antrag",
        parent: 'handover_conversation',
        section: "handover-conversation-section",
        icon: ICON_ASSIGN_AGENT,
        handler: this.onHandoverConversation,
      }];
      return [
        {
          id: 'handover_conversation',
          title: "Handover a conversation",
          section: this.$t('COMMAND_BAR.SECTIONS.CONVERSATION'),
          icon: ICON_ASSIGN_AGENT,
          children: chatStoryEntries.map(option => option.id),
        },
        ...chatStoryEntries,
      ];
    },
